...
if (!TOKEN || !AMOUNT_ARG) {
  console.log('Usage: node sell-fixed.mjs <TOKEN_CA> <amount|100%>');
  process.exit(1);
}
...
async function main() {
  console.log(`🦞 GIZMO SELL SYSTEM v2 - FIXING 403 ERROR`);
  ...
  // FIXED: Using Jupiter v6 API instead of deprecated lite-api
  ...
  console.log(`📡 Quote response: ${quoteResp.status} ${quoteText.substring(0, 200)}...`);
  
  let quote;
  try {
    quote = JSON.parse(quoteText);
  } catch (e) {
    console.error('❌ Failed to parse quote response:', quoteText);
  ...
  // FIXED: Using Jupiter v6 swap API
  ...
  console.log(`📡 Swap response: ${swapResp.status} ${swapText.substring(0, 200)}...`);
  
  if (!swapResp.ok) throw new Error('Swap failed: ' + swapText);
  ...
  if (conf.value.err) {
    ...
    console.log('✅ CONFIRMED — sold for ~' + outSol.toFixed(4) + ' SOL');
  }
}
